...
- [Perform Authorization with Moneytree](#perform-authorization-with-moneytree)
  ...
  - [PKCE](#pkce)
- [Perform Onboard with Moneytree (fast onboarding)](#perform-onboard-with-moneytree)
  - [Code Grant](#code-grant-1)
  - [PKCE](#pkce-1)
...
```
## Perform Onboard with Moneytree

### Code Grant		

`Onboard` feature behaves similar to [Authorization API Code Grant](#authorization-api-code-grant) except that it allows guests to sign up for a Moneytree account within clicks `without a password`, only an email address is required.		
...
`Onboard` feature behaves similar to [Authorization with PKCE](#authorization-with-pkce) except that it allows the guests to sign up for a Moneytree account within clicks `without a password`, only an email address is required.		
...
The access token to query Moneytree Link REST API is available after the user is authorized via `PKCE`.
