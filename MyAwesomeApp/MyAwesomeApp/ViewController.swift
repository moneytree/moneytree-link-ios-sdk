...
final class ViewController: UIViewController {
  ...
  @IBAction func onboardButtonTapped(_ sender: Any) {
    MLIntelligence.shared().recordEvent("[awesomeApp]onboard_tapped")
  ...
  @IBAction func itButtonTapped() {
    MLIntelligence.shared().recordEvent("[awesomeApp]isshōTsūchō_tapped")
    MTIsshoTsucho.shared.makeViewController { viewController, error in
      guard let itViewController = viewController else {
        self.handleError(error)
        return
      }
      self.navigationController?.pushViewController(itViewController, animated: true)
    }
  }
  ...
  func updateToken(with newToken: MTOAuthCredential?) {
    ...
    tokenLabel.text = "Access Token: \(token)"
    let expirationDate = newToken!.expirationDate
    let formatter = DateFormatter()
    formatter.dateStyle = .medium
    formatter.timeStyle = .long
    expiresInLabel.text = "Expires in: \(formatter.string(from: expirationDate))"
  }
}
